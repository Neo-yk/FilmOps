...
try:
    ...
    # Inline implementations for compatibility with newer transformers versions
    ...
    def prune_linear_layer(layer, index, dim=0):
        ...
        if layer.out_features == layer.in_features and dim == 1:
            new = type(layer)(layer.in_features, layer.out_features, bias=layer.bias is not None)
        else:
            new = type(layer)(layer.in_features, layer.out_features, bias=layer.bias is not None)
        new.weight = torch.nn.Parameter(W.clone().detach(), requires_grad=True)
        if layer.bias is not None:
            if dim == 0:
                b = layer.bias.clone().detach().index_select(0, index)
            else:
                b = layer.bias.clone().detach()
            new.bias = torch.nn.Parameter(b, requires_grad=True)
        return new
...
class BeitEmbeddings(nn.Module):
    ...
    def forward(
        ...
        interpolate_pos_encoding: Optional[bool] = None, # 后面版本加入的
    ) -> torch.Tensor:
